...
export class WorkflowDemoAgent {
  private agentOrchestrationClient: Client | null = null;
  private intentGraphClient: Client | null = null;
  private routerClient: Client | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    this.agentOrchestrationClient = new Client(
      { name: 'workflow-demo-agent', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    this.intentGraphClient = new Client(
      { name: 'workflow-demo-agent', version: '1.0.0' },
      { capabilities: {} }
    );
  ...
  async executeWorkflow(userQuery: string): Promise<WorkflowResult> {
    ...
    try {
      ...
      const orchestrationContent = orchestrationResult.content as any;
      ...
      console.log(`   Agents: ${orchestrationData.orchestration_card?.agents?.map((a: any) => a.name).join(', ') || 'N/A'}\n`);
      ...
      const graphContent = graphResult.content as any;
      ...
      const executionContent = executionResult.content as any;
  ...
  async cleanup(): Promise<void> {
    ...
    if (this.agentOrchestrationClient) await this.agentOrchestrationClient.close();
    if (this.intentGraphClient) await this.intentGraphClient.close();
    if (this.routerClient) await this.routerClient.close();
    if (this.memoryClient) await this.memoryClient.close();

...
The **Router MCP Server** is the execution engine of GAFF (Graphical Agentic Flow Framework). It takes intent graphs generated by the `intent-graph-generator` and executes them by orchestrating calls to agents, managing state through the Memory MCP server, and coordinating Human-in-the-Loop (HITL) interactions.
...
All execution state is stored in the Memory MCP server:
